...
const Login: React.FC<LoginProps> = ({ onLogin, onBack }) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Assuming Analytics is globally available or imported elsewhere
            // If not, you'll need to import it: import Analytics from 'your-analytics-library';
    ...
    const handleOAuthLogin = async (provider: 'google' | 'github') => {
        ...
        try {
            const { error } = await supabase.auth.signInWithOAuth({
                provider: provider,
                options: {
                    redirectTo: `${window.location.origin}/insightslm`
                }
            });
            if (error) throw error;
        } catch (err: any) {
        ...
        <div className="min-h-screen bg-[#F5F2EB] flex flex-col justify-center items-center p-4">
            ...
            <div className="w-full max-w-md bg-white border border-[#D6D1C7] shadow-xl p-8 md:p-12 relative overflow-hidden">
                ...
                <div className="mb-8 text-center">
                    <h1 className="text-3xl font-serif text-[#2C2A26] mb-2">InsightsLM</h1>
                    <p className="text-[#2C2A26]/60 font-sans text-sm">AI-Powered Knowledge & Audio Overviews</p>
                </div>
