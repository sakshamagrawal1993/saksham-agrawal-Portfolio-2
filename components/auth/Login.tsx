...
const Login: React.FC<LoginProps> = ({
    ...
    const effectiveRedirectPath = searchParams.get('redirect') || redirectPath;
    const { user } = useAuth();
    ...
    const handleOAuthLogin = async (provider: 'google' | 'github') => {
        ...
        try {
            const { error } = await supabase.auth.signInWithOAuth({
                provider: provider,
                options: {
                    redirectTo: `${window.location.origin}${effectiveRedirectPath}`
                }
            });
            if (error) throw error;
        } catch (err: any) {
        ...
        return (
            <div className="min-h-screen bg-[#F5F2EB] flex flex-col justify-center items-center p-4 animate-fade-in-up">
                ...
                <div className="w-full max-w-md bg-white border border-[#D6D1C7] shadow-xl p-8 md:p-12 relative overflow-hidden text-center">
                    ...
                    <button
                        onClick={() => navigate(effectiveRedirectPath)}
