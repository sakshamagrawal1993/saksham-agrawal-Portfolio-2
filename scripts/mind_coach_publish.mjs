...
const statePath = resolve(root, '.loop-mind-coach/STATE.json');
const outputPath = resolve(root, argValue('--output', '.loop-mind-coach/runs/publication/publication.json'));
...
if (!config.publishCommandsAllowed) {
  throw new Error('Publication is disabled in .loop-mind-coach/config.json');
}
if (!existsSync(statePath)) {
  throw new Error('Missing .loop-mind-coach/STATE.json; run the engineering loop first.');
}
...
if (!existsSync(resolve(n8nRepo, '.env')) || !existsSync(resolve(n8nRepo, 'tools/n8n-cli/n8n-cli'))) {
  stages.push({
    name: 'n8n preflight',
    ok: false,
    exitCode: null,
  ...
  const n8nDryRun = run('n8n mind coach dry run', '/bin/zsh', ['-lc', dryRunCommand], n8nRepo);
  ...
  if (n8nDryRun.ok || n8nDryRun.exitCode === 2) {
    ...
    stages.push(run('n8n mind coach apply', '/bin/zsh', ['-lc', applyCommand], n8nRepo));
    for (const workflowId of workflowIds) {
      ...
      stages.push(run(`n8n activate ${workflowId}`, '/bin/zsh', ['-lc', activateCommand], n8nRepo));
    }
  }
}

for (const functionName of config.supabaseFunctions || []) {
  assertSafeCommand(`npx supabase functions deploy ${functionName}`);
  stages.push(run(
...
if (changedMigrations.length > 0) {
  const unsafeMigration = changedMigrations.find(
    (path) => !/mind_coach|mind-coach/i.test(basename(path)),
  );
  if (unsafeMigration) {
    stages.push({
