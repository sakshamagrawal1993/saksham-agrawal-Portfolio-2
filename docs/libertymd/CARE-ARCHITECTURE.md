...
- **P2-09 · Client PDF delivery:** ReportView mounts Download into shared `data-libertymd-report-delivery-actions` (P2-08 email may share the slot). Two documents — patient summary + physician SOAP — built **in-browser** from the same normalized `report_data` view-model (`jspdf`, dynamic import). **No** proxy PDF action, **no** n8n PDF, **no** Storage upload of report body. Headers on both: UTC date + AI-generated / not a diagnosis + no licensed clinician review. Filenames `LibertyMD-{patient|soap}-YYYY-MM-DD.pdf` (UTC; non-clinical). Chooser Patient / SOAP / Both; **Both** is gesture-safe (patient downloads immediately; SOAP via second tap or ready links — never silent dual `a.click()`, never merged/ZIP). Soft-gate ungated (guest + saved). Telemetry: Lexicon client `LibertyMd report_delivery_requested` `{ method: 'download', emit_origin: 'client' }` after ≥1 Blob success — **not** in Postgres `PRODUCT_EVENT_NAMES`. Device matrix for iOS Safari / Chrome Android obtain = DoD+ / UNTESTABLE in `:ci`.
...
- **Photo upload (P4-06):** Anonymous OK (JWT consult owner). Proxy action `upload_photo` receives image bytes (base64), validates **jpeg/png/webp ≤ 5 MiB**, **server-side EXIF strip**, then **service_role** `storage.from('libertymd-care').upload` to `{consultation_id}/photo/{object_uuid}`. Access only via **short-TTL signed URLs** (`PHOTO_SIGNED_URL_TTL_SECONDS = 900`); **never** `getPublicUrl` / permanent public links. Durable SoT = **Storage path only** (no attachments table). Response: `{ path, object_uuid, content_type, signed_url, expires_in }` (+ analysis stub). Client session chip OK; never filename/path/EXIF in Mixpanel / `libertymd_product_events`. Upload failure = **technical** severity only and **never blocks** `send_message`. Lab report attach is **P4-07** (linked-only; separate from photo).
- **Photo analysis (P4-06 Done bar):** **Stub only** this ticket. Documented reuse **pattern**: service-role re-sign → future LibertyMD-configured vision webhook → structured JSON inside `libertymd-care-proxy`. Health Twin process-lab-report is not a runtime dependency (lab PDF/CSV → biomarker JSON; wrong modality and product boundary). No photo bytes into Interview/Diagnosis n8n; no user-facing clinical photo interpretation; no orphan LibertyMD vision workflow.
- **Lab report upload (P4-07):** **Login required** (anonymous → `403` + `code: 'sign_in_required'` + **zero** Storage put + **zero** attribution insert). Soft-gate report viewing stays ungated — lab login ≠ report auth wall. Proxy action `upload_lab` (JWT linked + consult ownership): required owned **active** `patient_id`; validates **PDF + jpeg/png/webp ≤ 10 MiB**; transport SoT = `file_base64` (preferred) with `image_base64` alias; **service_role** put to `{consultation_id}/lab/{object_uuid}` via `buildLibertyMdCarePath`; signed URL TTL **`LAB_SIGNED_URL_TTL_SECONDS = 900`**; never `getPublicUrl`. Durable attribution SoT = thin table **`libertymd_lab_uploads`** (`consultation_id`, `object_uuid`, `patient_id`, `path`, `content_type`, `analysis_status`, `structured_results`) — proxy service_role writer only; RLS on; no anon/authenticated DML. **Object-scoped attribution only** — never `UPDATE libertymd_consultations.patient_id` / `patient_snapshot` from lab upload (Q2A). Identifiers (**patient_name / dob / date_of_birth / mrn / address / phone**) never in queryable columns or `structured_results` keys (AC4). **Redact-before-model fail-closed (Q3A/S4):** OCR-and-drop / crop gate before any model/n8n receives bytes or a model-intended URL; if redaction unavailable → `analysis.status = stub|pending_redaction` and **zero model/n8n calls**. Eng Done this ship: **no model egress** (live OCR optional behind future config). Private-bucket originals may still contain identifiers on-disk — mitigations are login + P1-24 retention + redact-before-model, not “bucket is clean.” Taxonomy: **SELECT** reuse of `health_parameter_definitions` / `health_parameter_ranges` only (stub map OK); demography from attributed profile age/sex, never OCR DOB; no parallel LibertyMD dictionary; no HT `process-lab-report` / twin writes. User-facing mapped values carry explicit **unreviewed / pending clinical review** labeling (AC9) — interpretive clinical prose withheld until expert residual clears. Analysis/upload failure = `severity: 'technical'`, `consult_continues: true`; never blocks `send_message`. **No** new `PRODUCT_EVENT_NAMES` / Mixpanel lifecycle (closed set 18). Lane F photo ingest / EXIF / anon photo untouched. HIPAA/BAA silence — no compliance claims in lab copy.
- **Lab model egress + n8n retention honesty (P4-07 AC6):** This ship sends **no** lab bytes/OCR/structured analytes to a vision model or n8n lab webhook (`model_egress: false`). If/when analysis leaves stub, the model/provider must receive **only** a redacted artifact (or structured JSON after redact) — never unredacted PDF/image bytes. LibertyMD Interview/Diagnosis/Guardrail workflows disable successful/error/manual execution payload retention; host execution pruning is operator-controlled defense-in-depth — **not** a claim of zero third-party retention. Unknown host pruning posture must be treated as an egress risk.
- **P1-24 lab retention:** Lab objects use the same `{consultation_id}/lab/{object_uuid}` path contract; cleanup/orphan Edge keys first path segment only — expired/orphan consult lab files are deletable like photo. Lab objects are in P1-24 Storage cleanup scope.
