...
Deno.test('P1-24 AC2 · Edge Storage API remove + libertymd-care allow-list', () => {
  ...
  assertTrue(/from\(['"]libertymd_photo_analyses['"]\)/.test(edge), 'photo bookkeeping table')
  assertTrue(/raw_deleted_at:\s*removedAt/.test(edge), 'cleanup records raw deletion time')
  assertTrue(/path:\s*null/.test(edge), 'cleanup clears stale private path')
